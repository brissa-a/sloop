...
type HoverCardProps = { user: Exclude<TrpcOut['user']['byId'], null> } & MantineAvatarProps;

export const UserHoverCard = ({ user, ...remain }: HoverCardProps) => {
    ...
    return <Stack gap={0}>
        ...
        <Group>
            ...
            <Stack gap={0}>
                ...
                <Group>
                    <Stack gap={0}>
                        <Text size="xs" c='dimmed'>Copié par</Text>
                        <Text size='sm'>{user._count.copiedBy}</Text>
        ...
        <Flex my={10} wrap={'wrap'} justify={'center'}>
            ...
            {groups.map(({ slug, id, role }) => <Badge m={2} key={slug + "-" + role} ml={10} style={{ cursor: 'pointer' }}
                renderRoot={(props) => {
